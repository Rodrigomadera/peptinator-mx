...
(function () {
    ...
    const buyBlock = p.price === null
      ...
      : `<div class="modal-buy">
           ...
           <div class="buy-actions">
             <button class="btn btn-primary" data-action="pedir">▸ Solicitar pedido</button>
             <button class="btn btn-ghost" data-action="coa">Solicitar COA del lote</button>
           </div>
           <p class="mono buy-note">// El pago y envío se confirman por el canal de contacto (en activación).</p>
         </div>`;
    return `
  ...
  modal.addEventListener("click", e => {
    ...
    if (action === "pedir") {
      // Stripe Payment Link si está configurado; si no, sección de contacto
      const link = typeof stripeLinkPara === "function" && currentProduct
        ? stripeLinkPara(currentProduct.id)
        : null;
      if (link) {
        const sep = link.includes("?") ? "&" : "?";
        window.open(`${link}${sep}client_reference_id=${currentProduct.id}`, "_blank", "noopener");
        closeModal();
      } else {
        closeModal();
